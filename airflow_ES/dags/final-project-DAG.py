...
def fetch_data():
    ...
    df = pd.read_sql("SELECT * FROM public.raw_customer_churn", conn)
    
    # Save Data
    df.to_csv('/opt/airflow/dags/raw_customer_churn.csv', index=False)
...
def clean_data():
    ...
    '''
    # Read Data
    df = pd.read_csv('/opt/airflow/dags/raw_customer_churn.csv')
...
def post_elastic():
    ...
    df=pd.read_csv('/opt/airflow/dags/clean_customer_churn.csv')
